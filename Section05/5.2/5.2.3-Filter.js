...
const incompleteTasks = data.filter(item => item.completed ===false);
...
// filter for user6's incomplete tasks

const userSixincomplete = userSixTasks.filter(tasks => tasks.completed === false )

// filter for tasks with a title less than 24 characters long
const shorTitles = data.filter(task => task.title.length < 24);
console.log(shorTitles);
